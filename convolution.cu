...
// COMMENTS TO GRADER:
// In this assignment, I finished the Task 1 and Task 2 of computing convultion 
// using CUDA. 

// I compiled and ran the programm on the computer in Multimedia Lab 2A successfully.
// If any problems occur, please do not hesitate to contact with me since I have
// put lots of efforts in it.

// Thanks sincerely for the professor's help.
                               ...
                               float *output )
{
	...
	// Initialize 
	// For elements that have do not enough input in its neighborhood
	// output[i] will have value 0.0
    output[tid] = 0.0;

	// For elements that have enough input in its neighborhood 
    if ( tid >= filterRadius && tid <= (dataSize - filterRadius - 1) ){
		
		// Index of the first element for input data
		int begin = tid - filterRadius;

		// Comopute the convolution
        for ( int i = 0; i < filterWidth; i++ )
            output[tid] += filter[i] * data[begin + i];
	}
                               ...
                               float *output )
{
	...
	// Store the elements of filter in shared memory
	if ( tx < filterWidth )
		filterS[tx] = filter[tx];

	// Store the elements of input data in shared memory
	
	// For the first block, it does not store the elements of the previous block
	if ( blockIdx.x > 0 )
		dataS[tx] = data[tid - BLOCK_SIZE];
	
	// Store the elements of this block
	dataS[tx + BLOCK_SIZE] = data[tid];

	// For the last block, it does not store the elements of the last block
	if ( blockIdx.x < NUM_BLOCKS -1 )
		dataS[tx + BLOCK_SIZE * 2] = data[tid + BLOCK_SIZE];

	// Synchronize to ensure that the data of filterS and dataS has been loaded before the computation
	__syncthreads();
	...
	if ( tid >= filterRadius && tid <= (dataSize - filterRadius - 1) ){
		...
		// Index of the first element for input
		int begin = tx + BLOCK_SIZE - filterRadius;

		// Computer the convolution
		for (int i = 0; i < filterWidth; i++ )
			output[tid] += filterS[i] * dataS[begin + i];
	}
